...
import { fetchILP } from "../../api-calls/accreditation/accreditationAPI";
import { messageHandler } from '../../services/websocket/messageHandler';
import { useEffect, useMemo } from 'react';

export const useFetchILP = () => {
  ...
  const queryKey = useMemo(() => ['info-level-programs'], []);
  ...
  const queryFn = async ({ signal }) => {
    try {
      ...
      const res = await fetchILP({ signal });
  ...
  */
  ...
  return {
    accredInfoLevelPrograms: data || [], // Provide empty array as default to avoid undefined
