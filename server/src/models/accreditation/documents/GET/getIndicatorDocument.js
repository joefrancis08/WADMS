...
const getIndicatorDocument = async (data = {}, connection = null) => {
  const { 
    title,
    year,
    accredBody,
  ...
  const query = `
    ...
    JOIN subparam_indicator_mapping sim
      ON ad.subparam_indicator_mapping_id = sim.id
    JOIN indicator i
    ...
    WHERE ai.title = ?
      ...
      AND ad.subparam_indicator_mapping_id IS NOT NULL
  ...
  try {
    ...
    console.error('Error fetching indicator document:', error);
    throw error;
  }
};

export default getIndicatorDocument;